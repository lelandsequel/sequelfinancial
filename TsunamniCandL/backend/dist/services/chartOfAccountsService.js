...
class ChartOfAccountsService {
    ...
    static async createAccount(input) {
        try {
            ...
            const existing = await database_1.default.chartOfAccounts.findUnique({
            ...
            if (input.parentId) {
                const parent = await database_1.default.chartOfAccounts.findUnique({
     ...
     */
    static async getAllAccounts(page = 1, limit = 10, type, status, parentOnly = false) {
        try {
            ...
            const [accounts, total] = await Promise.all([
                database_1.default.chartOfAccounts.findMany({
                    where,
                    skip,
                    take: limit,
                ...
                database_1.default.chartOfAccounts.count({ where })
            ]);
     ...
     * Update account
     */
    static async updateAccount(id, input) {
        try {
            ...
            if (account.isSystem && input.status === accounting_1.AccountStatus.ARCHIVED) {
                return {
                    success: false,
     ...
     */
    static async getAccountHierarchy(type) {
        try {
            const where = { parentId: null };
            if (type)
                where.type = type;
            const rootAccounts = await database_1.default.chartOfAccounts.findMany({
                where,
                include: {
                    children: {
                        include: {
     ...
     */
    static async getNextAccountNumber(type) {
        try {
     ...
     */
    static async searchAccounts(query, type) {
        try {
            const where = {
                OR: [
                ...
                status: accounting_1.AccountStatus.ACTIVE
            };
            if (type)
                where.type = type;
            const accounts = await database_1.default.chartOfAccounts.findMany({
...
ChartOfAccountsService.ACCOUNT_RANGES = {
    [accounting_1.AccountType.ASSET]: { min: 1000, max: 1999 },
    [accounting_1.AccountType.LIABILITY]: { min: 2000, max: 2999 },
    [accounting_1.AccountType.EQUITY]: { min: 3000, max: 3999 },
    [accounting_1.AccountType.REVENUE]: { min: 4000, max: 4999 },
    [accounting_1.AccountType.EXPENSE]: { min: 5000, max: 5999 }
};
