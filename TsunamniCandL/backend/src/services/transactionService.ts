...
export class TransactionService {
   ...
   */
  static async createTransaction(input: CreateTransactionInput): Promise<ApiResponse<Transaction>> {
    try {
   ...
   */
  static async updateTransaction(id: string, input: UpdateTransactionInput): Promise<ApiResponse<Transaction>> {
    try {
      const transaction = await prisma.transaction.findUnique({
        where: { id }
      });
  ...
  static async getTransactionSummary(
    startDate?: Date,
    endDate?: Date
  ): Promise<ApiResponse<Array<{ type: TransactionType; count: number; totalAmount: Decimal }>>> {
    try {
      const where: any = {};
      if (startDate || endDate) {
        where.date = {};
   ...
   */
  static async getUnbalancedTransactions(): Promise<ApiResponse<Transaction[]>> {
    try {
   ...
   */
  static async balanceTransaction(id: string): Promise<ApiResponse<Transaction>> {
    try {
      const transaction = await prisma.transaction.findUnique({
        where: { id },
        include: {
      ...
      const entries = transaction.journalEntries.map((entry: any) => ({
        accountId: entry.accountId,
        debit: entry.debit,
        credit: entry.credit,
        description: entry.description
   ...
   */
  static async getAccountBalance(accountId: string, startDate?: Date, endDate?: Date): Promise<ApiResponse<Decimal>> {
    try {
      const where: any = {
        accountId,
        transaction: {
