...
import { LiabilityType } from '../types/accounting';
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await LiabilityService.getAllLiabilities(page, limit, activeOnly);
...
router.get('/total', async (req, res) => {
  try {
    const activeOnly = req.query.activeOnly !== 'false';
    const result = await LiabilityService.getTotalLiabilityAmount(activeOnly);
...
router.get('/due/:days', async (req, res) => {
  try {
    ...
    const result = await LiabilityService.getLiabilitiesDueWithin(days, activeOnly);
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await LiabilityService.getLiabilityById(id);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await LiabilityService.updateLiability(id, updateData);
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await LiabilityService.deleteLiability(id);
...
export { router as liabilityRoutes };
